...
namespace WebApp.Controllers
{
    ...
    public class RprBirimRaporlariController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Index(List<int> raporTipIDs, int? vaSurecId, int? birimId, bool export = false)
        {
            ...
            if (mmMessage.Messages.Count == 0)
            {
                if (export)
                {
                    var birimRapor = new BirimRaporHesaplama(vaSurecId.Value, raporTipIDs, birimId);
                    var veriler = birimRapor.Hesapla();
                    var gv = new GridView();
                    ...
                    return File(System.Text.Encoding.UTF8.GetBytes(stringWriter.ToString()), Response.ContentType, "Birim Rapor - Madde Veri Listesi.xls");
                }
                ...
                //    }
                //    var vaBirimIDs = qExp.Select(s => s.BirimID).Distinct().ToList();
                //    var vaSurecleriMaddeIDs = qExp.Select(s => s.VASurecleriMaddeID).Distinct().ToList();
                //    var girilenDegerler = qExp.Where(p => vaBirimIDs.Contains(p.BirimID) && vaSurecleriMaddeIDs.Contains(p.VASurecleriMaddeID)).SelectMany(s => s.VaSurecleriMaddeGirilenDegers).ToList();

                //    var qeData = (from s in qExp
                //                  join dg in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = (int?)null } equals
                //                                                new { dg.BirimID, dg.VASurecleriMaddeID, dg.VACokluVeriDonemID } into deg
                //                  from g in deg.DefaultIfEmpty()
                //                  join dg1 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 1 } equals
                //                                                 new { dg1.BirimID, dg1.VASurecleriMaddeID, VACokluVeriDonemID = dg1.VACokluVeriDonemID ?? 0 } into deg1
                //                  from g1 in deg1.DefaultIfEmpty()
                //                  join dg2 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 2 } equals
                //                                                new { dg2.BirimID, dg2.VASurecleriMaddeID, VACokluVeriDonemID = dg2.VACokluVeriDonemID ?? 0 } into deg2
                //                  from g2 in deg2.DefaultIfEmpty()
                //                  join dg3 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 3 } equals
                //                                          new { dg3.BirimID, dg3.VASurecleriMaddeID, VACokluVeriDonemID = dg3.VACokluVeriDonemID ?? 0 } into deg3
                //                  from g3 in deg3.DefaultIfEmpty()
                //                  join dg4 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 4 } equals
                //                                       new { dg4.BirimID, dg4.VASurecleriMaddeID, VACokluVeriDonemID = dg4.VACokluVeriDonemID ?? 0 } into deg4
                //                  from g4 in deg4.DefaultIfEmpty()
                //                  join dg5 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 5 } equals
                //                                       new { dg5.BirimID, dg5.VASurecleriMaddeID, VACokluVeriDonemID = dg5.VACokluVeriDonemID ?? 0 } into deg5
                //                  from g5 in deg5.DefaultIfEmpty()
                //                  join dg6 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 6 } equals
                //                                       new { dg6.BirimID, dg6.VASurecleriMaddeID, VACokluVeriDonemID = dg6.VACokluVeriDonemID ?? 0 } into deg6
                //                  from g6 in deg6.DefaultIfEmpty()
                //                  join dg7 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 7 } equals
                //                                       new { dg7.BirimID, dg7.VASurecleriMaddeID, VACokluVeriDonemID = dg7.VACokluVeriDonemID ?? 0 } into deg7
                //                  from g7 in deg7.DefaultIfEmpty()
                //                  join dg8 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 8 } equals
                //                                       new { dg8.BirimID, dg8.VASurecleriMaddeID, VACokluVeriDonemID = dg8.VACokluVeriDonemID ?? 0 } into deg8
                //                  from g8 in deg8.DefaultIfEmpty()
                //                  join dg9 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 9 } equals
                //                                       new { dg9.BirimID, dg9.VASurecleriMaddeID, VACokluVeriDonemID = dg9.VACokluVeriDonemID ?? 0 } into deg9
                //                  from g9 in deg9.DefaultIfEmpty()
                //                  join dg10 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 10 } equals
                //                                       new { dg10.BirimID, dg10.VASurecleriMaddeID, VACokluVeriDonemID = dg10.VACokluVeriDonemID ?? 0 } into deg10
                //                  from g10 in deg10.DefaultIfEmpty()
                //                  join dg11 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 11 } equals
                //                                       new { dg11.BirimID, dg11.VASurecleriMaddeID, VACokluVeriDonemID = dg11.VACokluVeriDonemID ?? 0 } into deg11
                //                  from g11 in deg11.DefaultIfEmpty()
                //                  join dg12 in girilenDegerler on new { s.BirimID, s.VASurecleriMaddeID, VACokluVeriDonemID = 12 } equals
                //                                       new { dg12.BirimID, dg12.VASurecleriMaddeID, VACokluVeriDonemID = dg12.VACokluVeriDonemID ?? 0 } into deg12
